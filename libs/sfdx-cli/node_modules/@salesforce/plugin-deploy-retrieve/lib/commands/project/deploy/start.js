...
export default class DeployMetadata extends SfCommand {
    ...
    async run() {
        ...
        // eslint-disable-next-line @typescript-eslint/require-await
        Lifecycle.getInstance().on('deployZipData', async (zipData) => {
            this.zipSize = zipData.zipSize;
            if (flags.verbose && this.zipSize)
                this.log(`Deploy size: ${getZipFileSize(this.zipSize)} of ~39 MB limit`);
            if (zipData.zipFileCount) {
                this.zipFileCount = zipData.zipFileCount;
                if (flags.verbose && this.zipSize)
                    this.log(`Deployed files count: ${this.zipFileCount} of 10,000 limit`);
            }
        });
